# ReadingMarker

中文 | [English](README.en.md)

一個為 iPhone/iPad Safari 寫的閱讀進度標記腳本。

主要用來解決在手機上看長篇小說 (如 AO3 或 FC2 部落格) 時，因為不小心重整網頁或切換 App 導致畫面跳回最頂端、找不到上次讀到哪的痛點。

---

## 功能特點

- **AO3 支援**：精準鎖定每篇作品的 `works ID`。
- **FC2 支援**：自動相容不同的子網域。
- **狀態提示**：
  - **■ 按鈕**：點擊記錄當前滾動位置；長按清空記錄。
  - **▶ 按鈕**：如果該文章本地有舊記錄，按鈕會自動變黑（點亮）；若無記錄則呈現半透明灰色。
- **介面文字會依系統語言顯示**

---

## 安裝

1. 手機確保已安裝並開啟 [Stay for Safari](https://apps.apple.com/tw/app/stay-for-safari-%E4%BD%BF%E7%94%A8%E8%80%85%E8%85%B3%E6%9C%AC%E8%88%87%E5%BB%A3%E5%91%8A%E6%94%94%E6%88%AA%E6%93%B4%E5%85%85/id1591620171)
2. 開啟 Stay App，點擊右上角 **「+」**，選擇 **通過鏈接導入**
3. 複製並貼上下方網址：

   ``` text
    https://raw.githubusercontent.com/rye1014/ReadingMarker/main/reading-marker.user.js
   ```

4. 進入 iPhone 設定 -> Safari -> 延伸功能，確保已將 Stay 啟用

---

## 使用

1. **短按 ■ 按鈕**：儲存目前閱讀位置。
2. **長按 ■ 按鈕**：*v1.1.0 新增* - 彈出確認視窗，可直接清空該頁面的閱讀記錄。
3. **滑動閱讀時**：*v1.2.0 新增* - 只顯示 ■ 按鈕。
4. **停止滑動後**：*v1.2.0 新增* - 自動展開兩個按鈕，點擊按鈕區域外任意位置可收回/展開。
5. **短按 ▶ 按鈕**：跳回上次儲存的位置。

---

## 畫面預覽

<img src="images/preview.jpg" width="320" alt="畫面預覽">

---

<details>
<summary><h2> 更新日誌 </h2></summary>

### [v1.2.1] - 2026-06-20

- **新增**：新增英語介面支援，補充雙語 metadata 與使用說明。

### [v1.2.0] - 2026-06-20

- **調整**：閱讀中預設收合為單一記錄按鈕，停止滑動後自動展開，並支援點擊按鈕區域外任意位置切換收合狀態。

### [v1.1.1] - 2026-06-07

- **修復**：解決長按清空紀錄後，`isLongPressed` 狀態鎖死導致無法立刻進行短按標記的 Bug。

### [v1.1.0] - 2026-06-07

- **新增**：長按標記按鈕 ■ 清空本頁記錄

### [v1.0.0] - 2026-06-07

- 基礎版本發佈，支援 AO3 與 FC2 部落和基本滾動位置記錄與跳轉。

</details>
